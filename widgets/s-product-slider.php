...
if ( class_exists( 'WooCommerce' ) ) {
    ...
    function s_woo_product_cat_list( ) {
...
// Widget Start here
class S_Product_Slider extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->add_control(
            'from',
            [
                'label' => __( 'Products from', 'shemuls-woo-addon' ),
                'type' => \Elementor\Controls_Manager::SELECT,
                'options' => [
                    'select'  => __( 'Select Products', 'shemuls-woo-addon' ),
                    'category'  => __( 'Select Categories', 'shemuls-woo-addon' )
                ],
                'default' => 'select'
            ]
        );
        ...
        $this->add_control(
            'p_ids',
            [
                'label' => __( 'And/Or Select products', 'shemuls-woo-addon' ),
                'type' => \Elementor\Controls_Manager::SELECT2,
                'multiple' => true,
                'options' => s_woo_product_list(),
                'condition' => [
                    'from' => 'select',
                ],
            ]
        ...
        $this->add_control(
            'cat_ids',
            [
                'label' => __( 'And/Or Categories', 'shemuls-woo-addon' ),
                'type' => \Elementor\Controls_Manager::SELECT2,
                'multiple' => true,
                'options' => s_woo_product_cat_list(),
                'condition' => [
                    'from' => 'category',
                ],
            ]
        ...
        $this->add_control(
			'swoo_post_per_page',
			[
				'label' => __( 'Post per page', 'shemuls-woo-addon' ),
	...
	protected function render() {
		...
		if($settings['from'] == 'category') {
            $q = new WP_Query( array(
                'posts_per_page' => $settings['swoo_post_per_page'], 
                'post_type' => 'product',
                'tax_query' => array(
                    array(
		...
		?>
			...
			<div class="s-woo-product-area">
				...
				<?php if($settings['from'] == 'category' && empty($settings['cat_ids'])) { ?>
                	<div class="alert alert-warning"><p>Please select product category</p></div> 
            	<?php } ?>
			</div>
